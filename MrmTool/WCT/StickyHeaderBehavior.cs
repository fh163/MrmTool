...
public class StickyHeaderBehavior : HeaderBehaviorBase
{
    ...
    protected override bool AssignAnimation()
    {
        if (base.AssignAnimation())
        {
            ...
            ExpressionAnimation? anim = _headerVisual?.Compositor.CreateExpressionAnimation("Max(propSetOffset.OffsetY - scrollPropSet.Translation.Y, 0)");
            anim?.SetReferenceParameter("propSetOffset", _animationProperties);
            anim?.SetReferenceParameter("scrollPropSet", _scrollProperties);

            _headerVisual?.StartAnimation("Offset.Y", anim);
